...
public static partial class RobloxApiMethods
{
    ...
    {
        RobloxOperation uncompletedOperation = (await client.ThrowIfNull().SendRequest(new GenerateUserThumbnailRequest()
        {
            UserId = userId,
            Size = size,
            Format = format,
        ...
        return (RobloxOperation) await uncompletedOperation.WaitForCompletionAsync(client)!;
    }
}